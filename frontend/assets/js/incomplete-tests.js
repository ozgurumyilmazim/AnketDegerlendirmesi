...
function viewTestDetail(testId) {
    ...
    if (viewAnswersBtn) {
        ...
        viewAnswersBtn.onclick = function() {
            // Close detail modal and open answers modal
            const detailModalEl = document.getElementById('incompleteDetailModal');
...
let currentAnswersTestItem = null;

async function openAnswersModal(testItem) {
    currentAnswersTestItem = testItem;
    const p = testItem.participants || {};
    const fullName = `${p.first_name || ''} ${p.last_name || ''}`.trim() || 'İsimsiz Katılımcı';
    ...
    document.getElementById('answersModalTitle').innerHTML = `
        ...
        <small class="fs-6 text-white-50 ms-2">(Oturum: ${testItem.session_code || '-'})</small>
    `;
    ...
    renderAnswersGrid(questions, testItem.test_answers || {});
    setupAnswersFilterEvents(questions, testItem.test_answers || {});
}
