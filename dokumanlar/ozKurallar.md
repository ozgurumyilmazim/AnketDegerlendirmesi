# ozKurallar.md - Ozgur Yilmaz Kuralları

Bu dosya icindeki kurallar , Ozgur Yilmaz'in gelistirdigi MMPI Psikolojik Test Sistemi icin yazilmistir. Bu dosya , LLM'ler icin yazilmistir. LLM'ler bu dosyayi okuduktan sonra , Ozgur Yilmaz'in gelistirdigi MMPI Psikolojik Test Sistemi'ni daha iyi anlayacaklardir. LLM'ler bu kurallara uymak zorundadir.

## Language Rules

LLM'ler bu kurallara uymak zorundadir.

- Do not generate code comments or commit messages in Turkish. Only commit in English.
...
- `tests/` — Playwright test suite (Chromium, headless)
- `dokumanlar/` — All documentation files (you are reading this file now).
- `belgeler` — These documents are in Turkish and should be read to understand the project better.
- `tasks` — The tasks defined for the LLM to work.
- `docker-compose.yaml` — Docker compose file for postgres, postgrest, nginx, redis
- `env.example` — Environment variables to copy as .env and modify according to the installation environment.
