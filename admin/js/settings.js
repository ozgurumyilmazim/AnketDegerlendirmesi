...
// Varsayılan ayarlar
    const defaultSettings = {
        kvkkTitle: 'KVKK ve Onam Metni',
        kvkkText: `<h4 class="text-center mb-4">MİNNESOTA ÇOK YÖNLÜ KİŞİLİK ENVANTERİ (MMPI) TESTİ</h4>
    ...
    async function loadKvkkSettings() {
        try {
            ...
            if (error) {
                ...
                showAlert('KVKK ayarları yüklenirken hata oluştu!', 'danger');
                return;
            }
...
// KVKK ayarlarını kaydet
    async function saveKvkkSettings() {
        try {
            ...
            showAlert('KVKK ayarları başarıyla kaydedildi!', 'success');
            
        } catch (error) {
            console.error('KVKK ayarları kaydedilirken hata:', error);
            showAlert('KVKK ayarları kaydedilirken hata oluştu!', 'danger');
        }
    }

// KVKK ayarlarını sıfırla
...
// Geriye uyumluluk için showAlert fonksiyonunu koruyalım
